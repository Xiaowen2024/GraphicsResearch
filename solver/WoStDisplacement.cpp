...
int main( int argc, char** argv ) {
   ...
   string shape = "rectangle-displacement";
   srand( time(NULL) );
   ...
   for( int j = 0; j < s; j++ )
   {
      ...
      for( int i = 0; i < s; i++ )
      {
         ...
         if( insideDomain(x0, boundaryDirichlet, boundaryNeumann) ){
            ...
            u = solve( x0, boundaryDirichlet, boundaryNeumann, deform_x);
            cout << u << std::endl;
         }
         out << u;
         if( i < s-1 ) out << ",";
   ...
   for (int i = 0; i < boundaryDirichlet[0].size(); i++) {
      ...
      double x_deformed = deform_x(point);
      outBoundaryD << x_deformed << "," << imag(point) << "," << 1 << std::endl;
   }
}
