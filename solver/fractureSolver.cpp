        ...
        for (const auto& polyline : boundary) {
            for (size_t i = 0; i + 1 < polyline.size(); ++i) {
                ...
                for (double t = 0.0; t < length; t += initialStep) {
                    ...
                    cout << "Round: " << round << ", Coarse point: " << real(coarsePoint) << ", " << imag(coarsePoint) << ",  gradient: " << real(coarseGradient[0]) << ", " << imag(coarseGradient[1]) << 
...
int main( int argc, char** argv ) {
    ...
    // Round: 0, Coarse point: 0.51, 0.21,  gradient: 1.03572, -0.127267, 0.0177103, -0.127267, Stress magnitude: 269.357

    int count = 0;
    vector<vector<Polyline>> boundaries = updateBoundaries(count, fileName, crackTips, boundaryDirichlet, boundaryNeumann, displacedBoundaryDirichlet, boundaryUpdateFile);
