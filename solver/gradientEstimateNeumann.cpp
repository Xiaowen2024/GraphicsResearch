...
// c++ -std=c++17 -O3 -pedantic -Wall gradientEstimateNeumann.cpp -o gradientEstimateNeumann -I /opt/homebrew/Cellar/eigen/3.4.0_1/include/eigen3 -w
...
int main( int argc, char** argv ) {
   string shape = "gradient_estimate_free_boundary_rect_fast_";
   // // // double size = 1; 
   double h = 0.01;
   ...
   // Vec2D solved = solve(Vec2D(1, 0.5), boundaryDirichlet, boundaryNeumann, deformFunc);
   ...
   // vector<Vec2D> gradient = solveGradient(Vec2D(0.5, 0.5), boundaryDirichlet, boundaryNeumann, displacement, gradientFile );
